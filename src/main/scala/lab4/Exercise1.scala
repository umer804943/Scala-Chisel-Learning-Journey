package lab4

import chisel3._
import chisel3.util._

object ALUOP {
  val ALU_ADD = 0.U(4.W)
  val ALU_SUB = 1.U(4.W)
  val ALU_AND = 2.U(4.W)
  val ALU_OR = 3.U(4.W)
  val ALU_XOR = 4.U(4.W)
  val ALU_SLT = 5.U(4.W)
  val ALU_SLL = 6.U(4.W)
  val ALU_SLTU = 7.U(4.W)
  val ALU_SRL = 8.U(4.W)
  val ALU_SRA = 9.U(4.W)
  val Branch_BEQ = 10.U(4.W)
  val Branch_BNE = 11.U(4.W)
  val Branch_BLT = 12.U(4.W)
  val Branch_BGE = 13.U(4.W)
  val Branch_BLTU = 14.U(4.W)
  val Branch_BGEU = 15.U(4.W)
}

trait Config {
// word length configuration parameter
  val WLEN = 32
// ALU operation control signal width
  val ALUOP_SIG_LEN = 4
}

class ALUIO extends Bundle with Config {
  val in_A = Input(UInt(WLEN.W))
  val in_B = Input(UInt(WLEN.W))
  val alu_Op = Input(UInt(ALUOP_SIG_LEN.W))
  val out = Output(UInt(WLEN.W))
  val sum = Output(UInt(WLEN.W))
}

class Exercise1 extends Module with Config {
  val io = IO(new ALUIO)

  val sum = io.in_A + Mux(io.alu_Op(0), -io.in_B, io.in_B)
  val cmp = Mux(
    io.in_A(WLEN - 1) === io.in_B(WLEN - 1),
    sum(WLEN - 1),
    Mux(io.alu_Op(1), io.in_B(WLEN - 1), io.in_A(WLEN - 1))
  )
  val shamt = io.in_B(4, 0).asUInt
  val shin = Mux(io.alu_Op(3), io.in_A, Reverse(io.in_A))
  val shiftr =
    (Cat(io.alu_Op(0) && io.in_A(WLEN - 1), shin).asSInt >> shamt)(WLEN - 1, 0)
  val shiftl = Reverse(shiftr)

  io.out := 0.U

  switch(io.alu_Op) {
    is(ALUOP.ALU_ADD) {
      io.out := sum
    }
    is(ALUOP.ALU_SUB) {
      io.out := sum
    }
    is(ALUOP.ALU_SLT) {
      io.out := cmp
    }
    is(ALUOP.ALU_SLTU) {
      io.out := cmp
    }
    is(ALUOP.ALU_SRA) {
      io.out := shiftr
    }
    is(ALUOP.ALU_SRL) {
      io.out := shiftr
    }
    is(ALUOP.ALU_SLL) {
      io.out := shiftl
    }
    is(ALUOP.ALU_AND) {
      io.out := (io.in_A & io.in_B)
    }
    is(ALUOP.ALU_OR) {
      io.out := (io.in_A | io.in_B)
    }
    is(ALUOP.ALU_XOR) {
      io.out := (io.in_A ^ io.in_B)
    }
    is(ALUOP.Branch_BEQ) {
      io.out := (io.in_A.asSInt === io.in_B.asSInt).asUInt
    }
    is(ALUOP.Branch_BNE) {
      io.out := (io.in_A.asSInt =/= io.in_B.asSInt).asUInt
    }
    is(ALUOP.Branch_BLT) {
      io.out := (io.in_A.asSInt < io.in_B.asSInt).asUInt
    }
    is(ALUOP.Branch_BGE) {
      io.out := (io.in_A.asSInt >= io.in_B.asSInt).asUInt
    }
    is(ALUOP.Branch_BLTU) {
      io.out := (io.in_A.asUInt < io.in_B.asUInt).asUInt
    }
    is(ALUOP.Branch_BGEU) {
      io.out := (io.in_A.asUInt >= io.in_B.asUInt).asUInt
    }
  }
  io.sum := sum
}
